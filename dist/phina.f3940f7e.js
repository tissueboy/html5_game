...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"phina.js":[function(require,module,exports) {
...
(function () {
  ...
  String.prototype.$method("format", function (arg) {
    // 置換ファンク
    var rep_fn = undefined; // オブジェクトの場合
...
(function () {
  ...
  Array.prototype.$method("flatten", function (level) {
    ...
    if (level) {
      ...
      arr = this.reduce(function (previousValue, curentValue) {
        return Array.isArray(curentValue) ? previousValue.concat(curentValue.flatten()) : previousValue.concat(curentValue);
      }, []);
    }
  ...
  Array.prototype.$method("toULElement", function () {// TODO: 
  });
  /**
   * @method toOLElement
  ...
  Array.prototype.$method("toOLElement", function () {// TODO:
  });
  /**
   * @method range
   * @static
...
phina.namespace(function () {
  ...
  phina.$method('createClass', function (params) {
    ...
    _class.prototype.$extend(params); // 継承用
    ...
    _class.prototype._creator.prototype = _class.prototype; // static property/method
...
phina.namespace(function () {
  ...
  phina.define('phina.geom.Vector2', {
     ...
     */
    distanceSquared: function distanceSquared(v) {
     ...
     */
    random: function random(min, max, len) {
     ...
     */
    fromAngle: function fromAngle(rad, len) {
      len = len || 1;
      this.x = Math.cos(rad) * len;
      this.y = Math.sin(rad) * len;
      return this;
     ...
     * @return {phina.geom.Vector2} ベクトル
     */
    fromDegree: function fromDegree(deg, len) {
     ...
     */
    rotate: function rotate(rad, center) {
    ...
    _static: {
       ...
       * @return {Number} 2点間の距離
       */
      distance: function distance(lhs, rhs) {
       ...
       * @return {Number} 2点間の距離の自乗
       */
      distanceSquared: function distanceSquared(lhs, rhs) {
       ...
       */
      manhattanDistance: function manhattanDistance(lhs, rhs) {
       ...
       */
      reflect: function reflect(v, normal) {
       ...
       * 補間（未実装）
       */
      slerp: function slerp(lhs, rhs, t) {// TODO:
        // cos...
      },
...
phina.namespace(function () {
  ...
  phina.define('phina.geom.Matrix33', {
    ...
    invert: function invert() {
      ...
      var m21 = this.m21;
      var m22 = this.m22;
      var det = this.determinant(); // |m00, m01, m02|
      // |m10, m11, m12|
      // |m20, m21, m22|
     ...
     */
    multiplyVector2: function multiplyVector2(v) {
...
phina.namespace(function () {
  ...
  phina.define('phina.geom.Rect', {
     ...
     */
    padding: function padding(top, right, bottom, left) {
...
phina.namespace(function () {
  ...
  phina.define('phina.geom.Collision', {
    _static: {
       ...
       */
      testCircleCircle: function testCircleCircle(circle0, circle1) {
       ...
       * @return {Boolean} 領域が重なっているかどうか
       */
      testRectRect: function testRectRect(rect0, rect1) {
       ...
       * @return {Boolean} 領域が重なっているかどうか
       */
      testCircleRect: function testCircleRect(circle, rect) {
        // まずは大きな矩形で判定(高速化)
        ...
        var c = phina.geom.Circle(circle.x, circle.y, circle.radius); // left top
       ...
       */
      testCircleLine: function testCircleLine(circle, p1, p2) {
        // 先に線分端との判定
        if (circle.contains(p1.x, p1.y) || circle.contains(p2.x, p2.y)) return true; // 半径の2乗

        var r2 = circle.radius * circle.radius; // 円の中心座標

        var p3 = phina.geom.Vector2(circle.x, circle.y); // 各ベクトル

        var p1p2 = phina.geom.Vector2.sub(p1, p2);
        var p1p3 = phina.geom.Vector2.sub(p1, p3);
        var p2p3 = phina.geom.Vector2.sub(p2, p3); // 外積

        var cross = phina.geom.Vector2.cross(p1p2, p1p3); // 外積の絶対値の2乗

        var cross2 = cross * cross; // p1p2の長さの2乗

        var length2 = p1p2.lengthSquared(); // 円の中心から線分までの垂線の距離の2乗

        var d2 = cross2 / length2; // 円の半径の2乗より小さいなら重複

        if (d2 <= r2) {
          var dot1 = phina.geom.Vector2.dot(p1p3, p1p2);
          var dot2 = phina.geom.Vector2.dot(p2p3, p1p2); // 通常は内積の乗算
...
phina.namespace(function () {
  ...
  phina.define('phina.util.EventDispatcher', {
    ...
    */
    flare: function flare(type, param) {
      var e = {
        type: type
      };
...
(function () {
  ...
  phina.define('phina.util.Tween', {
    ...
    fromTo: function fromTo(target, beginProps, finishProps, duration, easing) {
      this.target = target;
      this.beginProps = beginProps;
      this.finishProps = finishProps;
      this.duration = duration || 1000;
      this.easing = easing; // setup
    ...
    to: function to(target, finishProps, duration, easing) {
      var beginProps = {};
    ...
    from: function from(target, beginProps, duration, easing) {
      var finishProps = {};
    ...
    by: function by(target, props, duration, easing) {
      var beginProps = {};
      var finishProps = {};
    ...
    yoyo: function yoyo() {
      ...
      }, this); // TODO: easing も反転させる
      // this.easing = easing;

      return this;
    },
    gain: function gain(time) {
      this.seek(this.time + time);
    },
    forward: function forward(time) {
      this.seek(this.time + time);
    },
    backward: function backward(time) {
      this.seek(this.time - time);
    },
    ...
    _static: {
      ...
      EASING: {
        ...
        easeInOutBounce: function easeInOutBounce(t, b, c, d) {
          if (t < d / 2) return phina.util.Tween.EASING.easeInBounce(t * 2, 0, c, d) * .5 + b;else return phina.util.Tween.EASING.easeOutBounce(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
        },
...
(function () {
  ...
  phina.define('phina.util.Ticker', {
    ...
    run: function run() {
      var now = new Date().getTime(); // 1フレームに掛かった時間

      this.deltaTime = now - this.currentTime; // 全体の経過時間

      this.elapsedTime = now - this.startTime;
      var start = this.currentTime = now;
      this.flare('tick');
      var end = new Date().getTime(); // フレームを更新

      this.frame += 1; // calculate elapsed time

      var elapsed = end - start; // calculate next waiting time
    ...
    resume: function resume() {// TODO: 
    },
    stop: function stop() {// TODO: 
    },
    rewind: function rewind() {// TODO: 
    },
    _accessor: {
      fps: {
        "get": function get() {
...
phina.namespace(function () {
  ...
  phina.define('phina.util.ChangeDispatcher', {
    ...
    register: function register(key, defaultValue) {
      if (arguments.length === 1) {
        var obj = arguments[0];
...
(function () {
  ...
  phina.define('phina.util.Flow', {
    ...
    resolve: function resolve(arg) {
      this.resultValue = arg; // キューに積まれた関数を実行
    ...
    then: function then(func) {
      var self = this; // 成功ステータスだった場合は即実行
...
phina.namespace(function () {
  ...
  phina.define("phina.util.Color", {
     ...
     * 数値によるセッター.
     */
    setFromNumber: function setFromNumber(r, g, b, a) {
      this.r = r;
      this.g = g;
      this.b = b;
      this.a = a !== undefined ? a : 1.0;
     ...
     * 配列によるセッター
     */
    setFromArray: function setFromArray(arr) {
     ...
     * 文字列によるセッター
     */
    setFromString: function setFromString(str) {
     ...
     * CSS 用 16進数文字列に変換
     */
    toStyleAsHex: function toStyleAsHex() {
     ...
     * CSS 用 RGB文字列に変換
     */
    toStyleAsRGB: function toStyleAsRGB() {
     ...
     * CSS 用 RGBA文字列に変換
     */
    toStyleAsRGBA: function toStyleAsRGBA() {
    ...
    _static: {
       ...
       * hsla を rgba に変換
       */
      HSLAtoRGBA: function HSLAtoRGBA(h, s, l, a) {
       ...
       * rgb 値を作成
       */
      createStyleRGB: function createStyleRGB(r, g, b) {
        return "rgba(" + r + "," + g + "," + b + ")";
      },
       ...
       * rgba 値を作成
       */
      createStyleRGBA: function createStyleRGBA(r, g, b, a) {
        return "rgba(" + r + "," + g + "," + b + "," + a + ")";
      },
       ...
       * hsl 値を作成
       */
      createStyleHSL: function createStyleHSL(h, s, l) {
        return "hsl(" + h + "," + s + "%," + l + "%)";
      },
       ...
       * hsla 値を作成
       */
      createStyleHSLA: function createStyleHSLA(h, s, l, a) {
        return "hsla(" + h + "," + s + "%," + l + "%," + a + ")";
      }
    }
  });
...
phina.namespace(function () {
  ...
  phina.define("phina.util.Random", {
    ...
    randarray: function randarray(len, min, max) {
      len = len || 100;
      min = min || 0;
      max = max || 100;
...
phina.namespace(function () {
  ...
  phina.define('phina.util.QueryString', {
    _static: {
      parse: function parse(text, sep, eq, isDecode) {
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.AssetManager', {
    _static: {
      ...
      set: function set(type, key, asset) {
        if (!this.assets[type]) {
          this.assets[type] = {};
        }
      ...
      contains: function contains(type, key) {
        return;
      }
    }
  });
  ...
  phina.define('phina.asset.AssetLoader', {
    ...
    _static: {
      assetLoadFunctions: {
        ...
        text: function text(key, path) {
          var text = phina.asset.File();
          return text.load(path);
        }
      },
      register: function register(key, func) {
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.File', {
    ...
    _load: function _load(resolve) {
      ...
      } else if (_typeof(this.src) === 'object') {
      ...
      xml.send(null); // this.domElement = new Image();
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.Texture', {
    ...
    _load: function _load(resolve) {
      ...
      if (!isLocal && !/^data:/.test(this.src)) {// this.domElement.crossOrigin = 'Anonymous'; // クロスオリジン解除
      }
    ...
    transmit: function transmit(color) {
      // imagaオブジェクトをゲット
      var image = this.domElement; // 新規canvas作成

      var canvas = phina.graphics.Canvas().setSize(image.width, image.height); // 新規canvasに描画

      canvas.context.drawImage(image, 0, 0); // canvas全体のイメージデータ配列をゲット

      var imageData = canvas.context.getImageData(0, 0, canvas.width, canvas.height);
      var data = imageData.data; // 透過色の指定がなければ左上のrgb値を抽出

      var r = color !== undefined ? color.r : data[0];
      var g = color !== undefined ? color.g : data[1];
      var b = color !== undefined ? color.b : data[2]; // 配列を4要素目から4つ飛び（アルファ値）でループ
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.Sound', {
    ...
    play: function play(when, offset, duration) {
      if (this.source) {// TODO: キャッシュする？
      }
      ...
      source.playbackRate.value = this._playbackRate; // connect

      source.connect(this.gainNode);
      this.gainNode.connect(phina.asset.Sound.getMasterGain()); // play

      source.start(when ? when + this.context.currentTime : 0, offset || 0, duration); // check play end
    ...
    // 試してみるなう
    _oscillator: function _oscillator(type) {
      var context = this.context;
      var oscillator = context.createOscillator(); // Sine wave is type = “sine”
      ...
      this.source = oscillator; // connect

      this.source.connect(context.destination);
    },
    loadFromBuffer: function loadFromBuffer(buffer) {
      var context = this.context; // set default buffer
    ...
    setLoopStart: function setLoopStart(loopStart) {
      this.loopStart = loopStart;
      return this;
    },
    setLoopEnd: function setLoopEnd(loopEnd) {
    ...
    _loadFromFile: function _loadFromFile(r) {
      var self = this;
      var xml = new XMLHttpRequest();
      ...
      xml.onreadystatechange = function () {
        if (xml.readyState === 4) {
          if ([200, 201, 0].indexOf(xml.status) !== -1) {
            // 音楽バイナリーデータ
            var data = xml.response; // webaudio 用に変換
    ...
    _loadFromURIScheme: function _loadFromURIScheme(r) {
      var byteString = '';
    ...
    _static: {
      getMasterGain: function getMasterGain() {
        if (!this._masterGain) {
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.SoundManager', {
    _static: {
      ...
      muteFlag: false,
      currentMusic: null,
      play: function play(name, when, offset, duration) {
      ...
      pause: function pause() {// TODO: 
      },
      fade: function fade() {// TODO: 
      },
      ...
      playMusic: function playMusic(name, fadeTime, loop, when, offset, duration) {
      ...
      stopMusic: function stopMusic(fadeTime) {
        if (!this.currentMusic) {
          return;
        }
       ...
       * 音楽のボリュームを設定
       */
      setVolumeMusic: function setVolumeMusic(volume) {
       ...
       * 音楽のボリュームを取得
       */
      getVolumeMusic: function getVolumeMusic() {
...
phina.namespace(function () {
  ...
  phina.define('phina.asset.SpriteSheet', {
    ...
    _setupFrame: function _setupFrame(frame) {
    ...
    _setupAnim: function _setupAnim(animations) {
      this.animations = {}; // デフォルトアニメーション
...
phina.namespace(function () {
  ...
  phina.define("phina.asset.Font", {
    ...
    _checkLoaded: function _checkLoaded(font, callback) {
      var canvas = phina.graphics.Canvas();
      var DEFAULT_FONT = canvas.context.font.split(' ')[1];
      canvas.context.font = '40px ' + DEFAULT_FONT;
      var checkText = "1234567890-^\\qwertyuiop@[asdfghjkl;:]zxcvbnm,./\!\"#$%&'()=~|QWERTYUIOP`{ASDFGHJKL+*}ZXCVBNM<>?_１２３４５６７８９０－＾￥ｑｗｅｒｔｙｕｉｏｐａｓｄｆｇｈｊｋｌｚｘｃｖｂｎｍ，．あいうかさたなをん時は金なり"; // 特殊文字対応
      ...
      var checkLoadFont = function checkLoadFont() {
...
(function () {
  ...
  phina.define('phina.input.Input', {
    ...
    update: function update() {
      ...
      this.end = (this.now ^ this.last) & this.last; // 変化値を更新
      ...
      this.prevPosition.set(this.position.x, this.position.y); // 現在の位置を更新
    ...
    _start: function _start(x, y, flag) {
    ...
    _move: function _move(x, y) {
      ...
      this._tempPosition.y = y; // adjust scale
...
(function () {
  ...
  phina.define('phina.input.Mouse', {
     ...
     * ボタン取得
     */
    getButton: function getButton(button) {
     ...
     * ボタンダウン取得
     */
    getButtonDown: function getButtonDown(button) {
      if (typeof button === 'string') {
        button = BUTTON_MAP[button];
      }
     ...
     * ボタンアップ取得
     */
    getButtonUp: function getButtonUp(button) {
      if (typeof button == "string") {
        button = BUTTON_MAP[button];
      }
...
(function () {
  ...
  phina.define('phina.input.Touch', {
     ...
     * @constructor
     */
    init: function init(domElement, isMulti) {
      this.superInit(domElement);
      this.id = null;
     ...
     * タッチ開始時に true
     */
    getTouchStart: function getTouchStart() {
      return this.start != 0;
    },
     ...
     * タッチ終了時に true
     */
    getTouchEnd: function getTouchEnd() {
      return this.end != 0;
    }
  });
  /**
...
(function () {
  phina.define('phina.input.TouchList', {
    ...
    init: function init(domElement) {
      ...
      this.touches = [];
      var touchMap = this.touchMap = {}; // 32bit 周期でIDをループさせる
...
phina.namespace(function () {
  ...
  phina.define('phina.input.Keyboard', {
     ...
     * @returns {Boolean}   チェック結果
     */
    getKeyUp: function getKeyUp(key) {
      if (typeof key == "string") {
        key = phina.input.Keyboard.KEY_CODE[key];
      }
     ...
     */
    getKeyAngle: function getKeyAngle() {
      var angle = null;
     ...
     * 正規化されている
     */
    getKeyDirection: function getKeyDirection() {
     ...
     * キーの状態を設定する
     */
    setKey: function setKey(key, flag) {
      if (typeof key == "string") {
        key = phina.input.Keyboard.KEY_CODE[key];
      }
...
phina.namespace(function () {
  ...
  phina.define('phina.input.GamepadManager', {
     ...
     */
    dispose: function dispose(index) {
     ...
     */
    isConnected: function isConnected(index) {
      index = index || 0;
  ...
  phina.define("phina.input.Gamepad", {
     ...
     * ボタンを押した.
     */
    getKeyDown: function getKeyDown(button) {
      if (typeof button === 'string') {
        button = phina.input.Gamepad.BUTTON_CODE[button];
      }
     ...
     * ボタンを離した.
     */
    getKeyUp: function getKeyUp(button) {
      if (typeof button === 'string') {
        button = phina.input.Gamepad.BUTTON_CODE[button];
      }
     ...
     * スティックの入力されている方向.
     */
    getStickAngle: function getStickAngle(stickId) {
      stickId = stickId || 0;
     ...
     */
    getStickDirection: function getStickDirection(stickId) {
      stickId = stickId || 0;
     ...
     * @private
     */
    _updateState: function _updateState(gamepad) {
      this.id = gamepad.id;
     ...
     * @private
     */
    _updateStateEmpty: function _updateStateEmpty() {
     ...
     * @private
     */
    _updateButton: function _updateButton(value, buttonId) {
     ...
     * @private
     */
    _updateStick: function _updateStick(value, stickId, axisName) {
...
phina.namespace(function () {
  phina.define('phina.app.Updater', {
    ...
    _updateElement: function _updateElement(element) {
      var app = this.app; // 更新するかを判定

      if (element.awake === false) return; // エンターフレームイベント
      ...
      if (element.update) element.update(app); // タッチ判定
    ...
    _checkPoint: function _checkPoint(obj) {
    ...
    __checkPoint: function __checkPoint(obj, p) {
...
phina.namespace(function () {
  phina.define('phina.app.Interactive', {
    ...
    _checkElement: function _checkElement(element) {
      var app = this.app; // 更新するかを判定

      if (element.awake === false) return; // 子供を更新
    ...
    __checkPoint: function __checkPoint(obj, p) {
      ...
      if (overFlag) {
        if (p.getPointingStart()) {
          obj._touchFlags[p.id] = true;
          obj.flare('pointstart', e); // クリックフラグを立てる
...
phina.namespace(function () {
  ...
  phina.define('phina.app.BaseApp', {
    ...
    enableDatGUI: function enableDatGUI(callback) {
      if (phina.global.dat) {
    ...
    _loop: function _loop() {
      ...
      this.interactive.check(this.currentScene); // stats update
...
phina.namespace(function () {
  ...
  phina.define('phina.app.Element', {
    ...
    addChildTo: function addChildTo(parent) {
    ...
    getChildByName: function getChildByName(name) {// TODO: 
    },
...
phina.namespace(function () {
  ...
  phina.define('phina.app.Object2D', {
    ...
    hitTestRect: function hitTestRect(x, y) {
    ...
    hitTestCircle: function hitTestCircle(x, y) {
      // 円判定
      var p = this.globalToLocal(phina.geom.Vector2(x, y));
     ...
     * @param {Object} elm
     */
    hitTestElement: function hitTestElement(elm) {
      var rect0 = this;
      var rect1 = elm;
      return rect0.left < rect1.right && rect0.right > rect1.left && rect0.top < rect1.bottom && rect0.bottom > rect1.top;
    },
    globalToLocal: function globalToLocal(p) {
      var matrix = this._worldMatrix.clone();

      matrix.invert(); // matrix.transpose();

      var temp = matrix.multiplyVector2(p);
      return temp;
    },
    setInteractive: function setInteractive(flag, type) {
    ...
    setBoundingType: function setBoundingType(type) {
      this.boundingType = type;
      return this;
    },
    moveTo: function moveTo(x, y) {
    ...
    _calcWorldMatrix: function _calcWorldMatrix() {
      if (!this.parent) return; // cache check
      ...
      var world = this._worldMatrix; // ローカルの行列を計算
      ...
      local.m12 = this.position.y; // cache
      ...
      var b10 = parent.m10;
      var b11 = parent.m11;
      var b12 = parent.m12; // 親の行列と掛け合わせる
    ...
    _accessor: {
      ...
      width: {
        "get": function get() {
          return this.boundingType === 'rect' ? this._width : this._diameter;
        },
        "set": function set(v) {
          this._width = v;
        }
      ...
      height: {
        "get": function get() {
          return this.boundingType === 'rect' ? this._height : this._diameter;
        },
        "set": function set(v) {
          this._height = v;
        }
      ...
      radius: {
        "get": function get() {
          return this.boundingType === 'rect' ? (this.width + this.height) / 4 : this._radius;
        },
        "set": function set(v) {
          this._radius = v;
          this._diameter = v * 2;
      ...
      centerX: {
        ...
        "set": function set(v) {// TODO: どうしようかな??
        }
      },
...
phina.namespace(function () {
  phina.define('phina.app.Scene', {
    ...
    exit: function exit(nextLabel, nextArguments) {
      if (!this.app) return;
...
phina.namespace(function () {
  ...
  var Tweener = phina.define('phina.accessory.Tweener', {
    ...
    setUpdateType: function setUpdateType(type) {
      this.updateType = type;
      return this;
    },
    to: function to(props, duration, easing) {
      this._add({
        type: 'tween',
        mode: 'to',
        props: props,
    ...
    by: function by(props, duration, easing) {
      this._add({
        type: 'tween',
        mode: 'by',
        props: props,
    ...
    from: function from(props, duration, easing) {
      this._add({
        type: 'tween',
        mode: 'from',
        props: props,
    ...
    call: function call(func, self, args) {
      this._add({
        type: 'call',
        data: {
          func: func,
    ...
    moveTo: function moveTo(x, y, duration, easing) {
      return this.to({
        x: x,
        y: y
      }, duration, easing);
    },
    moveBy: function moveBy(x, y, duration, easing) {
      return this.by({
        x: x,
        y: y
      }, duration, easing);
    },
    rotateTo: function rotateTo(rotation, duration, easing) {
      return this.to({
        rotation: rotation
      }, duration, easing);
    },
    rotateBy: function rotateBy(rotation, duration, easing) {
      return this.by({
        rotation: rotation
      }, duration, easing);
    },
    scaleTo: function scaleTo(scale, duration, easing) {
      return this.to({
        scaleX: scale,
        scaleY: scale
      }, duration, easing);
    },
    scaleBy: function scaleBy(scale, duration, easing) {
      return this.by({
        scaleX: scale,
        scaleY: scale
      }, duration, easing);
    },
    fade: function fade(value, duration, easing) {
      return this.to({
        alpha: value
      }, duration, easing);
    },
    fadeOut: function fadeOut(duration, easing) {
      return this.fade(0.0, duration, easing);
    },
    fadeIn: function fadeIn(duration, easing) {
    ...
    _add: function _add(params) {
      this._tasks.push(params);
    },
    _updateTask: function _updateTask(app) {
      ...
      if (task.type === 'tween') {
        ...
        task.data.func.apply(task.data.self, task.data.args); // 1フレーム消費しないよう再帰

        this._update(app);
      } else if (task.type === 'set') {
        this.target.$extend(task.data.values); // 1フレーム消費しないよう再帰

        this._update(app);
      }
    },
    _updateTween: function _updateTween(app) {
    ...
    _updateWait: function _updateWait(app) {
      var wait = this._wait;
    ...
    _getUnitTime: function _getUnitTime(app) {
    ...
    _getDefaultDuration: function _getDefaultDuration() {
      var obj = UPDATE_MAP[this.updateType];
...
phina.namespace(function () {
  ...
  phina.define('phina.accessory.Draggable', {
    ...
    back: function back(time, easing) {
      if (time) {
        var t = this.target;
        ...
        }, time, easing || 'easeOutElastic').call(function () {
          tweener.remove();
...
phina.namespace(function () {
  ...
  phina.define('phina.accessory.Physical', {
    ...
    setFriction: function setFriction(fr) {
      this.friction = fr;
      return this;
    }
  });
...
(function () {
  ...
  Event.prototype.stop = function () {
    // イベントキャンセル
    this.preventDefault(); // イベント伝達を止める
...
(function () {
  ...
  MouseEvent.prototype.getter("pointX", function () {
    return this.clientX - this.target.getBoundingClientRect().left; // return this.pageX - this.target.getBoundingClientRect().left - window.scrollX;
  });
  /**
   * @method    pointY
   * マウスのY座標.
  ...
  MouseEvent.prototype.getter("pointY", function () {
    return this.clientY - this.target.getBoundingClientRect().top; // return this.pageY - this.target.getBoundingClientRect().top - window.scrollY;
  });
})();
...
(function () {
  ...
  TouchEvent.prototype.getter("pointX", function () {
    return this.touches[0].clientX - this.target.getBoundingClientRect().left; // return this.touches[0].pageX - this.target.getBoundingClientRect().left - tm.global.scrollX;
  });
  /**
   * @method    pointY
   * タッチイベント.
  ...
  TouchEvent.prototype.getter("pointY", function () {
    return this.touches[0].clientY - this.target.getBoundingClientRect().top; // return this.touches[0].pageY - this.target.getBoundingClientRect().top - tm.global.scrollY;
  });
})();
...
phina.namespace(function () {
  ...
  phina.define('phina.graphics.Canvas', {
    ...
    setSizeToScreen: function setSizeToScreen() {
    ...
    fitScreen: function fitScreen(isEver) {
    ...
    clearColor: function clearColor(fillStyle, x, y, width, height) {
      x = x || 0;
      y = y || 0;
      width = width || this.width;
      height = height || this.height;
     ...
     * ダッシュラインを描画
     */
    drawDashLine: function drawDashLine(x0, y0, x1, y1, pattern) {
      ...
      for (var i = 0; i < len; ++i) {
        if (patternTable[i % 16] == '1') {
          this.drawPoint(x, y); // this.fillRect(x, y, this.context.lineWidth, this.context.lineWidth);
        }
     ...
     */
    drawArrow: function drawArrow(x0, y0, x1, y1, arrowRadius) {
      var vx = x1 - x0;
      var vy = y1 - y0;
     ...
     * ラインストローク描画
     */
    strokeLines: function strokeLines() {
      this.beginPath();
     ...
     * 角丸四角形塗りつぶし
     */
    fillRoundRect: function fillRoundRect(x, y, width, height, radius) {
     ...
     * 角丸四角形ストローク描画
     */
    strokeRoundRect: function strokeRoundRect(x, y, width, height, radius) {
     ...
     * 塗りつぶし円弧を描画
     */
    fillArc: function fillArc(x, y, radius, startAngle, endAngle, anticlockwise) {
     ...
     * ストローク円弧を描画
     */
    strokeArc: function strokeArc(x, y, radius, startAngle, endAngle, anticlockwise) {
      return this.beginPath().arc(x, y, radius, startAngle, endAngle, anticlockwise).stroke();
    },
    pie: function pie(x, y, radius, startAngle, endAngle, anticlockwise) {
      var context = this.context;
      context.beginPath();
      context.moveTo(0, 0);
    ...
    fillPie: function fillPie(x, y, radius, startAngle, endAngle, anticlockwise) {
      return this.beginPath().pie(x, y, radius, startAngle, endAngle, anticlockwise).fill();
    },
    strokePie: function strokePie(x, y, radius, startAngle, endAngle, anticlockwise) {
     ...
     * ポリゴンパス
     */
    polygon: function polygon(x, y, size, sides, offsetAngle) {
     ...
     * ポリゴン塗りつぶし
     */
    fillPolygon: function fillPolygon(x, y, radius, sides, offsetAngle) {
     ...
     * ポリゴンストローク描画
     */
    strokePolygon: function strokePolygon(x, y, radius, sides, offsetAngle) {
     ...
     * star
     */
    star: function star(x, y, radius, sides, sideIndent, offsetAngle) {
      var x = x || 0;
      var y = y || 0;
     ...
     * 星を塗りつぶし描画
     */
    fillStar: function fillStar(x, y, radius, sides, sideIndent, offsetAngle) {
     ...
     * 星をストローク描画
     */
    strokeStar: function strokeStar(x, y, radius, sides, sideIndent, offsetAngle) {
     ...
     * heart
     */
    heart: function heart(x, y, radius, angle) {
      var half_radius = radius * 0.5;
      var rad = angle === undefined ? Math.PI / 4 : Math.degToRad(angle); // 半径 half_radius の角度 angle 上の点との接線を求める

      var p = Math.cos(rad) * half_radius;
      var q = Math.sin(rad) * half_radius; // 円の接線の方程式 px + qy = r^2 より y = (r^2-px)/q

      var x2 = -half_radius;
      var y2 = (half_radius * half_radius - p * x2) / q; // 中心位置調整

      var height = y2 + half_radius;
      var offsetY = half_radius - height / 2; // パスをセット
     ...
     * fill heart
     */
    fillHeart: function fillHeart(x, y, radius, angle) {
     ...
     * stroke heart
     */
    strokeHeart: function strokeHeart(x, y, radius, angle) {
    ...
    ellipse: function ellipse(x, y, w, h) {
      ...
      ctx.bezierCurveTo(xm - ox, ye, x, ym + oy, x, ym); // ctx.closePath();

      return this;
    },
    fillEllipse: function fillEllipse(x, y, width, height) {
      return this.beginPath().ellipse(x, y, width, height).fill();
    },
    strokeEllipse: function strokeEllipse(x, y, width, height) {
     ...
     * 中心に移動
     */
    transformCenter: function transformCenter() {
     ...
     * 画像として保存
     */
    saveAsImage: function saveAsImage(mime_type) {
      mime_type = mime_type || "image/png";
      var data_url = this.canvas.toDataURL(mime_type); // data_url = data_url.replace(mime_type, "image/octet-stream");

      window.open(data_url, "save"); // toDataURL を使えば下記のようなツールが作れるかも!!
    ...
    _static: {
      ...
      measureText: function measureText(font, text) {
...
phina.namespace(function () {
  ...
  phina.define('phina.graphics.CanvasRecorder', {
    ...
    _id: null,
    objectURL: null,
    init: function init(canvas, options) {
      this.superInit();
      this.canvas = canvas;
    ...
    start: function start(fps, recordingTime) {
      fps = fps || 30;
...
phina.namespace(function () {
  ...
  phina.define('phina.display.DisplayElement', {
     ...
     * @private
     */
    _calcWorldAlpha: function _calcWorldAlpha() {
      if (!this.parent) {
        this._worldAlpha = this.alpha;
        return;
      } else {
        var worldAlpha = this.parent._worldAlpha !== undefined ? this.parent._worldAlpha : 1.0; // alpha
...
phina.namespace(function () {
  ...
  var Shape = phina.define('phina.display.Shape', {
    ...
    calcCanvasWidth: function calcCanvasWidth() {
      return this.width + this.padding * 2;
    },
    calcCanvasHeight: function calcCanvasHeight() {
    ...
    isStrokable: function isStrokable() {
      return this.stroke && 0 < this.strokeWidth;
    },
    prerender: function prerender(canvas) {},
    postrender: function postrender(canvas) {},
    renderFill: function renderFill(canvas) {
      canvas.fill();
    },
    renderStroke: function renderStroke(canvas) {
      canvas.stroke();
    },
    render: function render(canvas) {
      var context = canvas.context; // リサイズ

      var size = this.calcCanvasSize();
      canvas.setSize(size.width, size.height); // クリアカラー

      canvas.clearColor(this.backgroundColor); // 中心に座標を移動

      canvas.transformCenter(); // 描画前処理

      this.prerender(this.canvas); // ストローク描画
      ...
      if (this.fill) {
        context.fillStyle = this.fill; // shadow の on/off
    ...
    _static: {
      watchRenderProperty: function watchRenderProperty(key) {
      ...
      watchRenderProperties: function watchRenderProperties(keys) {
    ...
    _defined: function _defined() {
      this.watchRenderProperties(['width', 'height', 'radius', 'padding', 'backgroundColor', 'fill', 'stroke', 'strokeWidth', 'shadow', 'shadowBlur']);
    }
  });
});
phina.namespace(function () {
...
phina.namespace(function () {
  var PathShape = phina.define('phina.display.PathShape', {
    ...
    addPaths: function addPaths(paths) {
      [].push.apply(this.paths, paths);
      this._dirtyDraw = true;
      return this;
    },
    addPath: function addPath(x, y) {
    ...
    changePath: function changePath(i, x, y) {
    ...
    _defined: function _defined() {
      phina.display.Shape.watchRenderProperties.call(this, ['lineCap', 'lineJoin']);
    },
    _static: {
      defaults: {
        fill: false,
...
phina.namespace(function () {
  ...
  phina.define('phina.display.Sprite', {
    superClass: 'phina.display.DisplayElement',
    init: function init(image, width, height) {
      this.superInit();
      this.srcRect = phina.geom.Rect();
      this.setImage(image, width, height);
    },
    draw: function draw(canvas) {
      var image = this.image.domElement; // canvas.context.drawImage(image,
    ...
    setImage: function setImage(image, width, height) {
    ...
    setFrameIndex: function setFrameIndex(index, width, height) {
...
phina.namespace(function () {
  ...
  phina.define('phina.display.DisplayScene', {
    ...
    init: function init(params) {
      ...
      this.gridY = phina.util.Grid(params.height, 16); // TODO: 一旦むりやり対応
...
phina.namespace(function () {
  phina.define('phina.display.CanvasRenderer', {
    ...
    renderChildren: function renderChildren(obj) {
      // 子供たちも実行
    ...
    renderObject: function renderObject(obj) {
      ...
      if (obj.clip) {
        ...
        context.clip();
        if (obj.draw) obj.draw(this.canvas); // 子供たちも実行
...
phina.namespace(function () {
  ...
  phina.define('phina.display.DomApp', {
    ...
    init: function init(options) {
      ...
      this.keyboard = phina.input.Keyboard(document); // 加速度センサーを生成

      this.accelerometer = phina.input.Accelerometer(); // ポインタをセット(PC では Mouse, Mobile では Touch)
      ...
      this.domElement.addEventListener(eventName, this._checkClick.bind(this)); // 決定時の処理をオフにする(iPhone 時のちらつき対策)
    ...
    _checkClick: function _checkClick(e) {
      var _check = function _check(element) {
        if (element.children.length > 0) {
...
phina.namespace(function () {
  ...
  phina.define('phina.ui.Button', {
    ...
    postrender: function postrender(canvas) {
      var context = canvas.context; // text
...
phina.namespace(function () {
  ...
  phina.define('phina.ui.Gauge', {
    ...
    setValue: function setValue(value) {
      value = Math.clamp(value, 0, this._maxValue); // end when now value equal value of argument

      if (this.value === value) return; // fire value change event
...
phina.namespace(function () {
  ...
  phina.define('phina.ui.CircleGauge', {
    ...
    postrender: function postrender() {// if (this.showPercentage) {
      //   // TODO:
...
phina.namespace(function () {
  ...
  var LabelArea = phina.define('phina.ui.LabelArea', {
    ...
    getTextWidthCache: function getTextWidthCache() {
      var cache = textWidthCache[this.font];
      return cache || (textWidthCache[this.font] = {});
    },
    spliceLines: function spliceLines(lines) {
      var rowWidth = this.width;
      var context = this.canvas.context;
      context.font = this.font;
      var cache = this.getTextWidthCache(); // update cache
      ...
      lines.forEach(function (line) {
        var str = '';
        var totalWidth = 0; // はみ出ていたら強制的に改行する
    ...
    _defined: function _defined() {
      var func = function func(newVal, oldVal) {
        if (this._lineUpdate === false && newVal !== oldVal) {
          this._lineUpdate = true;
        }
      };

      ['text', 'width', 'fontSize', 'fontWeight', 'fontFamily'].forEach(function (key) {
        this.$watch(key, func);
      }, this.prototype);
      phina.display.Shape.watchRenderProperties.call(this, ['verticalAlign', 'text', 'scroll', 'scrollX', 'scrollY']);
    },
...
phina.namespace(function () {
  ...
  phina.define('phina.game.ManagerScene', {
     ...
     * scenes をセット
     */
    setScenes: function setScenes(scenes) {
      this.scenes = scenes;
      this.sceneIndex = 0;
      return this;
    },
    replaceScene: function replaceScene(label, args) {
     ...
     */
    gotoScene: function gotoScene(label, args) {
      var index = typeof label == 'string' ? this.labelToIndex(label) : label || 0;
      var data = this.scenes[index];
     ...
     * 次のシーンへ飛ぶ
     */
    gotoNext: function gotoNext(args) {
      var data = this.scenes[this.sceneIndex];
      var nextIndex = null; // 次のラベルが設定されていた場合
     ...
     * ラベルからインデックスに変換
     */
    labelToIndex: function labelToIndex(label) {
     ...
     * インデックスからラベルに変換
     */
    indexToLabel: function indexToLabel(index) {
...
phina.namespace(function () {
  ...
  phina.define('phina.game.SplashScene', {
    ...
    _init: function _init() {
      ...
      this.sprite.alpha = 0;
      this.sprite.tweener.clear().to({
        alpha: 1
      }, 500, 'easeOutCubic').wait(1000).to({
        alpha: 0
      }, 500, 'easeOutCubic').wait(250).call(function () {
        this.exit();
      }, this);
    },
    _static: {
        ...
        children: {
          ...
          shareButton: {
            ...
            arguments: [{
              ...
              fontSize: 50,
              cornerRadius: 64,
              fill: 'rgba(240, 240, 240, 0.5)' // stroke: '#aaa',
              // strokeWidth: 2,
  ...
  phina.define('phina.social.Twitter', {
    ...
    _static: {
      ...
      defaults: {
        ...
        url: phina.global.location && phina.global.location.href // via: 'phi_jp',

      },
      createURL: function createURL(options) {
...
phina.namespace(function () {
  ...
  phina.define('phina.box2d.Box2dLayer', {
    ...
    _setupDebugDraw: function _setupDebugDraw() {
      // デバッグ用スプライト
    ...
    createBody: function createBody(params) {
    ...
    update: function update(app) {
      ...
      var positionIterations = 10; // 物理空間の更新
...
phina.namespace(function () {
  ...
  phina.define('phina.box2d.Box2dBody', {
    ...
    _setupBody: function _setupBody() {
      var self = this;
    ...
    _bindFixture: function _bindFixture() {
      var self = this;
      var target = this.target;
      ...
      var world = this.world;
      var scale = world._scale; // shape を取得
      ...
      fixture.shape = shape; // TODO: このへんは引数で指定できるようにする
...
},{}]},{},["node_modules/parcel/src/builtins/hmr-runtime.js","phina.js"], null)
//# sourceMappingURL=/phina.f3940f7e.js.map